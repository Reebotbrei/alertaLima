...
class ProfileGenderSelection extends StatelessWidget {
  ...
  const ProfileGenderSelection({Key? key, required this.viewModel}) : super(key: key);
...
class _ProfileGenderOption extends StatelessWidget {
  final String gender;
  const _ProfileGenderOption({Key? key, required this.gender}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    final viewModel = (context.findAncestorWidgetOfExactType<ProfileGenderSelection>() as ProfileGenderSelection).viewModel;
